...
if (!BOT_TOKEN) {
  console.error('❌ BOT_TOKEN تنظیم نشده! متغیر محیطی BOT_TOKEN رو چک کن.');
}
const TELEGRAM_API = `https://api.telegram.org/bot${BOT_TOKEN}`;
const redis = Redis.fromEnv();

const FONT_PATH = path.join(process.cwd(), 'fonts', 'Vazirmatn-Regular.ttf');
// چک اولیه‌ی وجود فونت - اگه روی سرورلس (مثل Vercel) دیپلوی می‌کنی و این فولدر
// جزو فایل‌های باندل‌شده نباشه، اینجا هشدار می‌گیری تا بفهمی مشکل از کجاست.
const FONT_EXISTS = fs.existsSync(FONT_PATH);
if (!FONT_EXISTS) {
  console.warn(
    `⚠️ فایل فونت پیدا نشد: ${FONT_PATH}\n` +
    'روی Vercel معمولاً باید مسیر فونت رو صریحاً در تنظیمات functions.includeFiles ' +
    'یا next.config.js -> outputFileTracingIncludes اضافه کنی وگرنه در دیپلوی نهایی نمی‌آد.'
  );
}
...
export default async function handler(req, res) {
  if (req.method !== 'POST') {
    res.status(200).json({ status: 'QR Bot is running ✅', fontFound: FONT_EXISTS });
    return;
  }
...
async function handleMessage(message) {
  ...
  // محدودیت طول متن برای جلوگیری از خطای ظرفیت کیوآرکد
  if (text.length > 1500) {
    await sendMessage(chatId, '⚠️ این متن خیلی طولانیه، لطفاً متن کوتاه‌تری بفرست (حداکثر حدود ۱۵۰۰ کاراکتر).');
    return;
  }
...
async function handleCallback(callback) {
  ...
  if (data === 'cap|yes') {
    ...
    await editMessage(chatId, messageId, '✏️ متنی که می‌خوای زیر کیوارکد نوشته بشه رو بفرست (حداکثر ۶۰ کاراکتر):', EMPTY_KEYBOARD);
  } else if (data === 'cap|no') {
    ...
    await editMessage(chatId, messageId, '⏳ در حال ساخت کیوارکد...', EMPTY_KEYBOARD);

    try {
      await generateFinal(chatId, session);
    } catch (err) {
      console.error('generateFinal fatal error:', err);
      await sendMessage(chatId, '❌ متأسفانه ساخت کیوارکد با خطا مواجه شد. لطفاً دوباره امتحان کن.');
    } finally {
      await clearSession(chatId);
    }
  } else {
...
async function generateFinal(chatId, session) {
  ...
  let qrBuffer;
  try {
    qrBuffer = await QRCode.toBuffer(session.text, {
      type: 'png',
      width: session.size,
      margin: 2,
    ...
    await sendMessage(chatId, '❌ نتونستم از این متن کیوارکد بسازم (احتمالاً خیلی طولانیه). یه متن کوتاه‌تر امتحان کن.');
    return;
  }
  ...
  try {
    ...
    // اگه ترکیب تصویر و متن به هر دلیلی خطا داد (مثلاً فونت پیدا نشد)، حداقل خود کیوارکد رو با کپشن تلگرام می‌فرستیم
    await sendPhoto(chatId, qrBuffer, `کیوارکدت آماده‌ست ✅\n\n${session.caption}`);
  }
}
...
async function tgFetch(endpoint, body) {
  ...
  if (!res.ok || (data && data.ok === false)) {
    console.error(`Telegram API error on ${endpoint}:`, data || res.status);
  }
  return data;
}

async function sendMessage(chatId, text, replyMarkup) {
  return tgFetch('sendMessage', { chat_id: chatId, text, parse_mode: 'Markdown', reply_markup: replyMarkup });
}

async function editMessage(chatId, messageId, text, replyMarkup) {
  return tgFetch('editMessageText', { chat_id: chatId, message_id: messageId, text, reply_markup: replyMarkup });
}

async function answerCallback(callbackId, text) {
  return tgFetch('answerCallbackQuery', { callback_query_id: callbackId, text });
}
...
async function sendPhoto(chatId, buffer, caption) {
  ...
  return tgFetch('sendPhoto', formData);
}
